...
const MaintenancePlans = () => (
  <>
    <Seo
      ...
      description="Subscription-style home maintenance plans for Texas homeowners — Essential, Summit, and Estate."
    ...
    <PageHero
      ...
      title={
        <>
          Care for your home,{" "}
          <em className="italic font-light text-accent">on autopilot.</em>
        </>
      }
    ...
    <section className="container-px mx-auto max-w-7xl py-20 md:py-28">
      <div className="grid gap-6 lg:grid-cols-3">
        {PLANS.map((p, i) => (
          <Reveal key={p.name} delay={i * 0.07}>
            <div
              className={`relative flex h-full flex-col rounded-3xl border bg-card p-8 hover-lift ${p.featured ? "border-accent shadow-accent" : "border-border/60"}`}
            >
              {p.featured && (
      ...
      <div className="mt-16 rounded-3xl bg-gradient-warm border border-border/60 p-8 text-center sm:p-10">
        <SectionEyebrow>Need something custom?</SectionEyebrow>
        <h2 className="font-display text-3xl font-medium">
          We'll build a plan around your property.
        </h2>
        <p className="mx-auto mt-3 max-w-xl text-muted-foreground">
          Multi-property, HOA, and light commercial plans available on request.
        </p>
